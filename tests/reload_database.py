...
async def load_profile_pictures():
    ...
    try:
        ...
        # Get all users
        users = await conn.fetch("SELECT id, first_name, last_name, email FROM users ORDER BY id")
        ...
        async with httpx.AsyncClient() as client:
            for user in users:
                user_id = user['id']
                print(f"🔄 Processing user {user_id}: {user['first_name']} {user['last_name']}")
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        profile_picture_id = await conn.fetchval("""
                            ...
                            RETURNING id
                        """, user_id, f"profile_{user_id}.jpg", "image/jpeg", len(image_data), image_data)
