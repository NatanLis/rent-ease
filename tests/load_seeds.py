...
async def clear_tables(session: AsyncSession):
    ...
    await session.execute(
        text("DELETE FROM users WHERE email != 'admin@rent-ease.com'")
    )
...
async def load_files(session: AsyncSession):
    ...
    for pdf_file in pdf_files:
        ...
        if not pdf_file.name.endswith(":Zone.Identifier"):
            sample_files_config.append(
                {"filename": pdf_file.name, "mimetype": "application/pdf"}
            )
    ...
    for file_config in sample_files_config:
        ...
        if file_path.exists():
            ...
            print(
                f"      Please add {file_config['filename']} to the tests/seeds/sample_files/ directory"
            )
...
async def load_seeds():
    ...
    async with async_session() as session:
        try:
            ...
            # Load users
            print("📝 Loading users...")
            with open("tests/seeds/users_test_data.json") as f:
            ...
            with open("tests/seeds/properties_test_data.json") as f:
            ...
            for prop_data in properties_data:
                ...
                if not owner_result:
                    print(
                        f"⚠️ Warning: Owner with email {prop_data['owner_email']} not found, skipping property '{prop_data['title']}'"
                    )
                    continue
            ...
            result = await session.execute(
                text("SELECT id, title, owner_id FROM properties ORDER BY id")
            )
            ...
            # Load units
            print("🏢 Loading units...")
            with open("tests/seeds/units_test_data.json") as f:
            ...
            for unit_data in units_data:
                print(
                    f"   🔍 Processing unit: {unit_data['name']} for property: {unit_data['property_title']}"
                )
                ...
                if not property_result:
                    print(
                        f"⚠️ Warning: Property with title '{unit_data['property_title']}' not found, skipping unit '{unit_data['name']}'"
                    )
                    continue
            ...
            result = await session.execute(
                text("SELECT id, name, property_id FROM units ORDER BY id")
            )
            ...
            with open("tests/seeds/leases_test_data.json") as f:
            ...
            for lease_data in leases_data:
                ...
                if not unit_result:
                    print(
                        f"⚠️ Warning: Unit '{lease_data['unit_name']}' in property '{lease_data['property_title']}' not found, skipping lease"
                    )
                    continue
                ...
                if not tenant_result:
                    print(
                        f"⚠️ Warning: Tenant with email {lease_data['tenant_email']} not found, skipping lease"
                    )
                    continue
                ...
                try:
                    start_date = datetime.strptime(
                        lease_data["start_date"], "%Y-%m-%d"
                    ).date()
                    end_date = (
                        datetime.strptime(lease_data["end_date"], "%Y-%m-%d").date()
                        if lease_data["end_date"]
                        else None
                    )
                except ValueError as e:
                    print(
                        f"⚠️ Warning: Invalid date format in lease data: {e}, skipping lease"
                    )
                    continue
