...
async def load_files(session: AsyncSession):
    ...
    for pdf_file in pdf_files:
        ...
        if not pdf_file.name.endswith(":Zone.Identifier"):
            sample_files_config.append({"filename": pdf_file.name, "mimetype": "application/pdf"})
        ...
        try:
            ...
            for prop_data in properties_data:
                ...
                if not owner_result:
                    print(
                        (
                            f"⚠️ Warning: Owner with email {prop_data['owner_email']} not found, "
                            f"skipping property '{prop_data['title']}'"
                        )
                    )
                    continue
            ...
            for unit_data in units_data:
                ...
                if not property_result:
                    print(
                        (
                            f"⚠️ Warning: Property with title '{unit_data['property_title']}' not found, "
                            f"skipping unit '{unit_data['name']}'"
                        )
                    )
                    continue
            ...
            for lease_data in leases_data:
                ...
                """
                    ...
                    {"unit_name": lease_data["unit_name"], "property_title": lease_data["property_title"]},
                )
                ...
                if not unit_result:
                    print(
                        (
                            f"⚠️ Warning: Unit '{lease_data['unit_name']}' in property "
                            f"'{lease_data['property_title']}' not found, skipping lease"
                        )
                    )
                    continue
