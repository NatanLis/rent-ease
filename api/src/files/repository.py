...
class FileRepository:
    ...
    async def create_file(self, file_data: FileCreate) -> File:
        """Create a new file."""
        file_obj = File(filename=file_data.filename, mimetype=file_data.mimetype, size=file_data.size, data=file_data.data)
    ...
    async def delete_file(self, file_id: int) -> bool:
        """Delete file by ID."""
        result = await self.session.execute(delete(File).where(File.id == file_id))
        await self.session.commit()
